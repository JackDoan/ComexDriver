...
void main(void)
{
    ...
    myPwm1 = PWM_init((void *)PWM_ePWM3_BASE_ADDR, sizeof(PWM_Obj));
...
void InitEPwmTimer()
{
    ...
    PWM_setPeriod(myPwm1, PWM3_TIMER_MIN);
    PWM_setPhase(myPwm1, 0x00000000);
    PWM_setActionQual_Period_PwmA(myPwm1, PWM_ActionQual_Toggle);
...
interrupt void ecap1_isr(void)
{
    ...
    if(CAP_getCap2(myCap) > PWM_getPeriod(myPwm1)*2+1 || CAP_getCap2(myCap) < PWM_getPeriod(myPwm1)*2-1)
    {
        Fail();
    }

    if(CAP_getCap3(myCap) > PWM_getPeriod(myPwm1)*2+1 || CAP_getCap3(myCap) < PWM_getPeriod(myPwm1)*2-1)
    {
        Fail();
    }

    if(CAP_getCap4(myCap) > PWM_getPeriod(myPwm1)*2+1 || CAP_getCap4(myCap) < PWM_getPeriod(myPwm1)*2-1)
    {
        Fail();
    }
    ...
    if(EPwm3TimerDirection == EPwm_TIMER_UP) {
        if(PWM_getPeriod(myPwm1) < PWM3_TIMER_MAX) {
            PWM_setPeriod(myPwm1, PWM_getPeriod(myPwm1) + 1);
        }
        else {
        ...
        if(PWM_getPeriod(myPwm1) > PWM3_TIMER_MIN) {
            PWM_setPeriod(myPwm1, PWM_getPeriod(myPwm1) - 1);
        }
        else {
            EPwm3TimerDirection = EPwm_TIMER_UP;
